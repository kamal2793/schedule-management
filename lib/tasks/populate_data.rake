  ...
  task populate_availability: :environment do
    ...
    events.each do |event|
      if event.recurrence_type == 'daily'
        ...
        EventManager.new(event, date).create_availabilities_for_system if (days_diff % step) == 0
        ...
        if days_diff >= 0
          ...
          EventManager.new(event).create_availabilities_for_system if (days_diff % step) == 0
        end
      end
    end
  end
